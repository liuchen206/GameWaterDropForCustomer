...
cc.Class({
  ...
  properties: {
    ...
    joystickType: {
      default: JoystickCommon.JoystickType.FULLSCREEN,
  ...
  onLoad() {
    ...
    if (this.joystickType == JoystickCommon.JoystickType.FOLLOW || this.joystickType == JoystickCommon.JoystickType.FULLSCREEN) {
      this.node.opacity = 0;
    }
    if(this.player!=null){
      this.playerJS = this.player.getComponent('Player');
    }
    this.playerStreeingForce = cc.Vec2.ZERO;
  },
  ...
  _touchStartEvent(event) {
    ...
    } else if (this.joystickType === JoystickCommon.JoystickType.FOLLOW || this.joystickType == JoystickCommon.JoystickType.FULLSCREEN) {

      // 记录摇杆位置，给 touch move 使用
      this._stickPos = touchPos;
      if(this.joystickType === JoystickCommon.JoystickType.FULLSCREEN){
        this.node.opacity = 0;
      }else{
  ...
  _touchMoveEvent(event) {
    ...
    if (this._radius > distance) {
      ...
      if(this.playerJS != null){
        this.playerStreeingForce = p.mul(this.playerJS.MaxForce);
      }
    } else {
      ...
      if(this.joystickType == JoystickCommon.JoystickType.FULLSCREEN){
        //摇杆超过极限距离 更改摇杆的位置,使得摇杆和ring始终在手边
        const touchPosInNode = this.node.convertToNodeSpaceAR(event.getLocation());
        this.ring.setPosition(touchPosInNode.add(p.neg().mul(this._radius)));
        this._stickPos = this.ring.getPosition();
      }
    }
  },
  ...
  GetForce(){
    return this.playerStreeingForce;
  },
});
