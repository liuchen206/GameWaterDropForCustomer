...
cc.Class({
    ...
    properties: {
        ...
        radiusForSearchCommonWaterDrop:{
            default: 400,
            tooltip: 'ai 检索普通水珠的半径',
        },
        radiusForSearchPlayerAI:{
            default: 350,
            tooltip: 'ai 检索其他ai的半径',
        },
        radiusForSearchPlayer:{
            default: 300,
            tooltip: 'ai 检索玩家的半径',
        },
        LogicPerferAttack:{
            default: false,
            tooltip: 'ai 是具有攻击性的还是防守性的',
        },
        isDebug:false,
        // blob数据
        quality:{
    ...
    onLoad () {
        ...
        this.manulPlayerContainer = cc.find('Canvas/GameView/GameWorld/PlayerContainer');
        // this.quality = 16;
        this.SteeringBehaviorsJS = this.node.getComponent('StreeBehavForPlayerAI');
    ...
    faceActCheck(){
        ...
        if(actName != this.currentActName){
            this.playerFaceAct(actName,amimatePlayTimes);
    ...
    update (dt) {
                ...
                if(tempDistance < anCheckBlobJS.radius*0.9){
                    if(anCheckBlobJS.radius < this.radius){
                        if(anCheckBlobJS.nodes.length > 0 && anCheckBlobJS.canntSplitTime <= 0){
                            ...
                            isCollided = true;
                            this.isMerge = true;
                            EventCenter.dispatchEvent(EventCenter.EventType.Someone_Be_Defeaded,{winerName:this.playerName,
        ...
        // 手动玩家 融合检测
        var allManulPlayerNodeList = this.getAllManulPlayerDrop(9999); 
        for(var i= 0;i<allManulPlayerNodeList.length;i++){
            var anCheckBlobJS = allManulPlayerNodeList[i].getComponent('Player');
            //检测半径是否重叠
            // var distanceWithTwoNode = anCheckBlobJS.node.position.sub(this.node.position).mag();
            // if(distanceWithTwoNode <= this.radius + anCheckBlobJS.radius){
            //     if(this.radius > anCheckBlobJS.radius){
    ...
    updateBlob(){
        var currentGraphicsStyle = graphicsStyle[1];
        if(this.isDebug){
        ...
        for(var i = 0;i < this.nodes.length;i++){
            ...
            nodeData.pos.x += (newNodePos.x - nodeData.pos.x)*0.02;
            nodeData.pos.y += (newNodePos.y - nodeData.pos.y)*0.02;
        }
        // 绘制节点结果
    ...
    merge(nodeTomergeJS){
        ...
        if(this.collectedWaterDropScore % 10 == 0){
            this.isDeyi = true;
        }
    },
    Killed(){
    ...
    split(){
        ...
        var newWaterDropNodeJS = newWaterDropNode.getComponent('PlayerAI');
    ...
    brains(){
        var commonWaterDropInViewList = this.getAllCommonWaterDrop(this.radiusForSearchCommonWaterDrop + this.radius);
        var otherPlayerAIList = this.getAllOtherPlayerDrop(this.radiusForSearchPlayerAI + this.radius);
        var PlayerList = this.getAllManulPlayerDrop(this.radiusForSearchPlayer + this.radius);
         ...
         * */ 
        // 检测是否能够进行某种行为
        var canCollectCommonWaterDrop = false;
        var canPursuitPlayer = false;
        var canEvadePlayer = false;

        var threatenPlayer = null;
        var weakPlayer = null;
        var nearestRadiusCommonWaterDrop = null;

        var tempdistance = 9999;
        for(var i = 0;i < commonWaterDropInViewList.length;i++){
            var commonWaterDropJS = commonWaterDropInViewList[i].getComponent("CommonWaterDrop");
            var dis = commonWaterDropJS.node.position.sub(this.node.position).mag();
            if(dis < tempdistance){
                tempdistance = dis;
                nearestRadiusCommonWaterDrop = commonWaterDropJS;
                canCollectCommonWaterDrop = true;
            }
        }
        for(var i = 0;i < otherPlayerAIList.length; i++){
            var playerAIJS = otherPlayerAIList[i].getComponent("PlayerAI");
            if(playerAIJS.radius >= this.radius){
                canEvadePlayer = true;
                threatenPlayer = playerAIJS;
            }else{
                canPursuitPlayer = true;
                weakPlayer = playerAIJS;
            }
        }
        for(var i = 0;i < PlayerList.length;i++){
            var playerJS = PlayerList[i].getComponent("Player");
            if(playerJS.radius >= this.radius){
                canEvadePlayer = true;
                threatenPlayer = playerJS;
            }else{
                canPursuitPlayer = true;
                weakPlayer = playerJS;
            }
        }
        var logicActionNow = PlayerAIAction.DoLikeAFool;
        if(this.LogicPerferAttack){
            // do attack thing
            if(canPursuitPlayer){
                logicActionNow = PlayerAIAction.PursuitPlayer;
            }else if(canCollectCommonWaterDrop){
                logicActionNow = PlayerAIAction.CollectCommonWaterDrop;
            }else if(canEvadePlayer){
                logicActionNow = PlayerAIAction.EvadePlayer;
            }
        }else{
            // do defend thing
            if(canCollectCommonWaterDrop){
                logicActionNow = PlayerAIAction.CollectCommonWaterDrop;
            }else if(canEvadePlayer){
                logicActionNow = PlayerAIAction.EvadePlayer;
            }else if(canPursuitPlayer){
                logicActionNow = PlayerAIAction.PursuitPlayer;
            }
        }
        this.resetStreeingBehaviors();
        if(logicActionNow == PlayerAIAction.DoLikeAFool){
            this.doLikeFool();
        }else if(logicActionNow == PlayerAIAction.CollectCommonWaterDrop){
            this.CollectCommonWaterDrop(nearestRadiusCommonWaterDrop.node);
        }else if(logicActionNow == PlayerAIAction.PursuitPlayer){
            this.PursuitPlayer(weakPlayer);
        }else if(logicActionNow == PlayerAIAction.EvadePlayer){
            this.EvadePlayer(threatenPlayer);
        }

        // cc.log("logicActionNow",logicActionNow);
    },
    resetStreeingBehaviors(){
        this.SteeringBehaviorsJS.beEvade = false;
        this.SteeringBehaviorsJS.beSeek = false;
        this.SteeringBehaviorsJS.beWander = false;
        this.SteeringBehaviorsJS.bePursuit = false;
        this.SteeringBehaviorsJS.seekTarget = null;
        this.SteeringBehaviorsJS.evadeTarget = null;
        this.SteeringBehaviorsJS.pursuitTarget = null;
    },
    doLikeFool(){
        this.SteeringBehaviorsJS.beEvade = false;
        this.SteeringBehaviorsJS.beSeek = false;
        this.SteeringBehaviorsJS.beWander = true;
        this.SteeringBehaviorsJS.bePursuit = false;
    },
    CollectCommonWaterDrop(commonWaterDropNode){
        this.SteeringBehaviorsJS.beEvade = false;
        this.SteeringBehaviorsJS.beSeek = true;
        this.SteeringBehaviorsJS.beWander = false;
        this.SteeringBehaviorsJS.bePursuit = false;
        this.SteeringBehaviorsJS.seekTarget = commonWaterDropNode;
    },
    PursuitPlayer(playerNodeJS){
        this.SteeringBehaviorsJS.beEvade = false;
        this.SteeringBehaviorsJS.beSeek = false;
        this.SteeringBehaviorsJS.beWander = false;
        this.SteeringBehaviorsJS.bePursuit = true;
        this.SteeringBehaviorsJS.pursuitTarget = playerNodeJS;
    },
    EvadePlayer(playerNodeJS){
        this.SteeringBehaviorsJS.beEvade = true;
        this.SteeringBehaviorsJS.beSeek = false;
        this.SteeringBehaviorsJS.beWander = false;
        this.SteeringBehaviorsJS.bePursuit = false;
        this.SteeringBehaviorsJS.evadeTarget = playerNodeJS;
    },
    ...
    getAllManulPlayerDrop(radius){
        if(this.manulPlayerContainer == null){
            return [];
        }
        var allNodeInCanvas = this.manulPlayerContainer.children;
        var playerList = [];
        for(var i = 0;i < allNodeInCanvas.length;i++){
            if(allNodeInCanvas[i].getComponent('Player') !== null){
                if(allNodeInCanvas[i].getComponent('Player').InstanceID !== this.InstanceID){
                    if(allNodeInCanvas[i].position.sub(this.node.position).mag()<=radius){
                        playerList.push(allNodeInCanvas[i]);
                    }
                }
